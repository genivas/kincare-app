...
const Dashboard = () => {
  ...
  const getUrgencyConfig = (level) => {
    switch(level) {
      case 'danger': return { text: 'Atenção Urgente', color: 'var(--danger-color)', dot: 'var(--danger-color)' };
      case 'warning': return { text: 'Em Observação', color: 'var(--warning-color)', dot: 'var(--warning-color)' };
      default: return { text: 'Saúde Estável', color: 'var(--text-light)', dot: 'var(--success-color)' };
    }
  };
    ...
    <div className="page-content" style={{paddingBottom: '80px', paddingTop: '3rem'}}>
      ...
      {showAlertModal && (
        <div style={{position: 'fixed', top:0, left:0, right:0, bottom:0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
          <div className="glass-card m-4 text-center" style={{padding: '2rem'}}>
            ...
            <h3 style={{fontSize: '1.4rem', marginBottom: '0.5rem'}}>Alerta Disparado</h3>
            <p className="mb-4" style={{color: 'var(--text-light)'}}>O grupo da família no WhatsApp foi notificado com sucesso.</p>
            <button className="btn-primary" onClick={() => setShowAlertModal(false)} style={{padding: '1rem', fontSize: '1.1rem', borderRadius: '12px'}}>Concluir</button>
          </div>
        </div>
      )}
      ...
      {showStatusModal && (
        <div style={{position: 'fixed', top:0, left:0, right:0, bottom:0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
          <div className="glass-card m-4" style={{width: '90%', maxWidth: '320px', padding: '2rem'}}>
            <h3 className="mb-4 text-center">Atualizar Status</h3>
            <div className="flex flex-col gap-3">
              <button className="btn-secondary" style={{color: 'var(--success-color)', border: 'none', background: 'var(--success-light)', padding: '1rem', borderRadius: '12px'}} onClick={() => changeStatus('success')}>🟢 Estável</button>
              <button className="btn-secondary" style={{color: 'var(--warning-color)', border: 'none', background: 'var(--warning-light)', padding: '1rem', borderRadius: '12px'}} onClick={() => changeStatus('warning')}>🟡 Em Observação</button>
              <button className="btn-secondary" style={{color: 'var(--danger-color)', border: 'none', background: 'var(--danger-light)', padding: '1rem', borderRadius: '12px'}} onClick={() => changeStatus('danger')}>🔴 Emergência</button>
              <button className="btn-secondary mt-2" style={{border: 'none'}} onClick={() => setShowStatusModal(false)}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="px-4 mb-6 flex justify-between items-start">
        <div className="flex items-center gap-4">
          <div style={{position: 'relative'}}>
            <img src={patient?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${patient?.name || 'Idoso'}`} alt="Avatar" className="avatar" style={{width: '64px', height: '64px', borderRadius: '24px', boxShadow: '0 8px 16px rgba(0,0,0,0.08)', border: 'none'}} />
          ...
          <div className="flex-1">
            <h1 style={{fontSize: '1.4rem', fontWeight: '700', letterSpacing: '-0.5px', marginBottom: '0.2rem', lineHeight: '1.2'}}>{patient?.name || 'Paciente'}</h1>
      ...
      <main className="px-4">
        ...
        <section className="mb-6">
          <h2 style={{fontSize: '1.1rem', fontWeight: '600', marginBottom: '0.75rem', color: 'var(--text-color)'}}>Prioridade</h2>
          {nextMed ? (
            <div className="glass-card" style={{background: 'var(--primary-color)', color: 'white', padding: '1.5rem', border: 'none', boxShadow: '0 10px 30px rgba(37, 99, 235, 0.2)'}}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  <span style={{fontSize: '0.85rem', opacity: 0.8, textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '600'}}>Próximo Medicamento</span>
            ...
            <div className="glass-card flex items-center gap-3" style={{padding: '1.5rem', background: 'var(--success-light)', border: 'none'}}>
              ...
              <div>
                <strong style={{color: 'var(--success-color)', fontSize: '1.1rem'}}>Tudo certo!</strong>
                <p style={{color: '#065f46', fontSize: '0.9rem'}}>Sem medicamentos pendentes.</p>
              </div>
            </div>
          )}
        </section>
        ...
        <section className="flex gap-3 mb-6">
          <div className="glass-card flex-1 mb-0" style={{padding: '1.25rem'}}>
            <span style={{fontSize: '0.8rem', color: 'var(--text-light)', fontWeight: '600', textTransform: 'uppercase'}}>Plantão</span>
            <div className="flex items-center gap-3 mt-3">
              <img src={todayCaregiver?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=Family`} alt={todayCaregiver?.name || 'Família'} style={{width: '44px', height: '44px', borderRadius: '14px', objectFit: 'cover'}}/>
              <div>
                <strong style={{fontSize: '1rem', display: 'block', color: 'var(--primary-color)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '80px'}}>{todayCaregiver?.name || 'Família'}</strong>
                <span style={{fontSize: '0.8rem', color: 'var(--text-light)'}}>Responsável hoje</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 flex-1">
            <div className="glass-card flex items-center justify-between mb-0" style={{padding: '1rem'}}>
              <div>
                <span style={{fontSize: '0.75rem', color: 'var(--text-light)', display: 'block', fontWeight: '600', textTransform: 'uppercase'}}>Pressão</span>
            ...
            <div className="glass-card flex items-center justify-between mb-0" style={{padding: '1rem'}}>
              <div>
                <span style={{fontSize: '0.75rem', color: 'var(--text-light)', display: 'block', fontWeight: '600', textTransform: 'uppercase'}}>Água</span>
        ...
        <section className="mb-6">
          <div className="flex justify-between items-center mb-3">
            <h2 style={{fontSize: '1.1rem', fontWeight: '600', color: 'var(--text-color)'}}>Eventos Recentes</h2>
